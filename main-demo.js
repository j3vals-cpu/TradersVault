...
function setupAutoLaunch() {
  // DEMO BUILD: auto-launch disabled for demo builds
}
...
const DEMO_BANNER_CSS = `
  body::before {
    content: "⚠ DEMO BUILD — Changes here do not affect production";
    display: block;
    position: fixed;
    top: 0;
    left: 0;
    right: 0;
    z-index: 999999;
    background: linear-gradient(90deg, #b8860b, #e8b84b, #b8860b);
    color: #07070a;
    text-align: center;
    padding: 4px 0;
...
function createWindow() {
  ...
  // Inject demo banner CSS after page loads
  mainWindow.webContents.on('did-finish-load', () => {
    mainWindow.webContents.insertCSS(DEMO_BANNER_CSS);
  });
...
function createTray() {
  ...
  const contextMenu = Menu.buildFromTemplate([
    { label: 'Traders Vault [DEMO]', enabled: false },
    { type: 'separator' },
    ...
    { type: 'separator' },
    { label: 'Quit Traders Vault Demo', click: () => { app.isQuitting = true; app.quit(); } }
  ]);

  tray.setToolTip('Traders Vault [DEMO]');
...
ipcMain.handle('pop-out-panel', (_, panelId, bounds) => {
  ...
  const popWin = new BrowserWindow({
    ...
    resizable: true,
    movable: true,
    title: 'Traders Vault [DEMO] — ' + panelId,
    webPreferences: {
      preload: path.join(__dirname, 'preload.js'),
      contextIsolation: true,
      nodeIntegration: false,
  ...
  // Inject demo banner into pop-out windows too
  popWin.webContents.on('did-finish-load', () => {
    popWin.webContents.insertCSS(DEMO_BANNER_CSS);
  });
...
// ─── DOWNLOAD UPDATE ─────────────────────────────────────────
// DEMO BUILD: auto-updater / download-update still available but
// the renderer's version check is pointed at the "demo" channel
// (handled in app_versions on the platform side). This handler
// remains so manual downloads still work if triggered.
ipcMain.handle('download-update', async (_, url) => {
  ...
  if (!fileName || !fileName.toLowerCase().endsWith('.exe')) {
    ...
    fileName = verMatch
      ? 'Traders.Vault.Demo.Setup.' + verMatch[1] + '.exe'
      : 'TradersVaultDemo-update.exe';
  }
...
ipcMain.on('open-ctrader-oauth', (event) => {
  ...
  const authWin = new BrowserWindow({
    width: 800, height: 700, title: 'Connect cTrader [DEMO]',
